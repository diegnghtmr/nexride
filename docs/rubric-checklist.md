...
Commit: HEAD on main (tag v0.1.10-mvp)
CI Run: see v0.1.10-mvp CI run on main (7/7 green)
...
| # | Criterio rúbrica | Estado | Evidencia | Nota |
|---|------------------|--------|-----------|------|
| CI1 | Completitud del pipeline: build + test + análisis estático automáticos en cada push/PR; fallos distinguibles por tipo | Completo | `ci.yml:3-7` triggers automáticos. 7 jobs separados con nombres distintos (lint-typecheck, architecture-rules, unit-tests, integration-tests, performance-smoke, security-static, build-openapi) — cada fallo identifica la dimensión. Build incluido en `build-openapi` (`ci.yml:218`). Análisis estático: ESLint + tsc + dependency-cruiser + npm audit + gitleaks. `scripts/verify-doc-paths.sh` y `scripts/verify-doc-consistency.sh` wired como steps en lint-typecheck. | v0.1.10-mvp: 7/7 verde (CI run on main, tag v0.1.10-mvp). |
...
Estos hallazgos fueron evaluados en la auditoría v8 (8th Judgment Day) y formalmente diferidos. Cerrados en v0.1.10-mvp (ver sección de resoluciones abajo).
...
| F4 | INFO | `.env.example` con claves muertas y valores incorrectos | Claves renombradas, valor `FLEET_MINIMUM_RESERVE_PCT` corregido a decimal 0.15, nuevas claves añadidas. `.env.example.new` generado (fallback path — `.env*` write-denied). | Resuelto en v0.1.10-mvp — ver `.env.example.new` |
| F5 | INFO | `AuditAction` TS union incluía `'ACTIVATE'` nunca emitido en código | Tipo narrowed a `'CREATE' \| 'UPDATE' \| 'DEACTIVATE' \| 'DELETE'`; DB CHECK constraint no migrado (sin riesgo, ADR-v9-01). | Resuelto en v0.1.10-mvp — `src/safe-points/infrastructure/safe-point-audit.entity.ts:3` |
| F7 | INFO | Analytics handler no instrumentaba fallos de persistencia | Counter `analytics_persist_failures_total{event_name}` añadido al registry y al catch del handler. | Resuelto en v0.1.10-mvp — `src/common/observability/metrics.registry.ts:147` (counter) + `src/analytics/handlers/dispatch.handler.ts:136` (inc) |
| F9 | INFO | Branch 23505 en `DecisionRepository` silenciosa (no log) | `logger.warn({ requestId, code: '23505' }, ...)` añadido antes de `return;`. | Resuelto en v0.1.10-mvp — `src/dispatch/infrastructure/persistence/decision.repository.ts:47` |
