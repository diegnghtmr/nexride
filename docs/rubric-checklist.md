...
Commit: HEAD on main (tag v0.1.16-mvp; históricamente v0.1.15-mvp / v0.1.14-mvp / v0.1.13-mvp / v0.1.12-mvp / v0.1.11-mvp)
CI Run: see v0.1.16-mvp CI run on main (7/7 green; históricamente v0.1.15-mvp 7/7 green)
...
|---|------|--------|-----------|------|
| E1 | Repositorio de código fuente con código + pruebas + CI antes de la evaluación | Completo | `src/` (vertical slice Dispatch+SafePoints+Fleet+Trip+Analytics), `test/` (unit + integration + architecture + performance), `.github/workflows/ci.yml` | Tag `v0.1.16-mvp` en `main` (históricamente: v0.1.0..v0.1.10 evolución base → v0.1.11..14 cerraron residuals 11°/12°/13° judgments → v0.1.15-mvp cerró 14° (Redis cache real + AbortSignal) → `v0.1.16-mvp` cierra residuals del 15° judgment day: F4 ioredis lifecycle (OnModuleDestroy), F2 pino messageKey='message', F1 matrix NFR-15 sync, F3 ADR-011 dispatch:config defer). |
...
| # | Criterio rúbrica | Estado | Evidencia | Nota |
|---|------------------|--------|-----------|------|
| CI1 | Completitud del pipeline: build + test + análisis estático automáticos en cada push/PR; fallos distinguibles por tipo | Completo | `ci.yml:3-7` triggers automáticos. 7 jobs separados con nombres distintos (lint-typecheck, architecture-rules, unit-tests, integration-tests, performance-smoke, security-static, build-openapi) — cada fallo identifica la dimensión. Build incluido en `build-openapi` (`ci.yml:218`). Análisis estático: ESLint + tsc + dependency-cruiser + npm audit + gitleaks. `scripts/verify-doc-paths.sh` y `scripts/verify-doc-consistency.sh` wired como steps en lint-typecheck. | v0.1.16-mvp: 7/7 verde (CI run on main, tag v0.1.16-mvp; históricamente v0.1.15-mvp y v0.1.14-mvp también 7/7 verde). |
