...
| F7 | INFO | Analytics handler no instrumentaba fallos de persistencia | Counter `analytics_persist_failures_total{event_name}` añadido al registry y al catch del handler. | Resuelto en v0.1.10-mvp — `src/common/observability/metrics.registry.ts`, `src/analytics/handlers/dispatch.handler.ts` |
| F9 | INFO | Branch 23505 en `DecisionRepository` silenciosa (no log) | `logger.warn({ requestId, code: '23505' }, ...)` añadido antes de `return;`. | Resuelto en v0.1.10-mvp — `src/dispatch/infrastructure/persistence/decision.repository.ts` |
